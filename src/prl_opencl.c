...
static enum prl_stat_entry clcommand_to_stat_entry(cl_int command) {
	switch (command) {
		case CL_COMMAND_NDRANGE_KERNEL                 : return stat_gpu_NDRANGE_KERNEL   ;
		case CL_COMMAND_TASK                             :return stat_gpu_TASK    ;
		case CL_COMMAND_NATIVE_KERNEL                    :return stat_gpu_NATIVE_KERNEL    ;
		case CL_COMMAND_READ_BUFFER                      :return stat_gpu_READ_BUFFER    ;
		case CL_COMMAND_WRITE_BUFFER                     :return stat_gpu_WRITE_BUFFER    ;
		case CL_COMMAND_COPY_BUFFER                      :return stat_gpu_COPY_BUFFER    ;
		case CL_COMMAND_READ_IMAGE                       :return stat_gpu_READ_IMAGE   ;
		case CL_COMMAND_WRITE_IMAGE                      :return stat_gpu_WRITE_IMAGE   ;
		case CL_COMMAND_COPY_IMAGE                       :return stat_gpu_COPY_IMAGE   ;
		case CL_COMMAND_COPY_IMAGE_TO_BUFFER             :return stat_gpu_COPY_IMAGE_TO_BUFFER  ;
		case CL_COMMAND_COPY_BUFFER_TO_IMAGE             :return stat_gpu_COPY_BUFFER_TO_IMAGE   ;
		case CL_COMMAND_MAP_BUFFER                       :return stat_gpu_MAP_BUFFER   ;
		case CL_COMMAND_MAP_IMAGE                        :return stat_gpu_MAP_IMAGE         ;
		case CL_COMMAND_UNMAP_MEM_OBJECT                 :return stat_gpu_UNMAP_MEM_OBJECT   ;
		case CL_COMMAND_MARKER                           :return stat_gpu_MARKER     ;
		case CL_COMMAND_ACQUIRE_GL_OBJECTS               :return stat_gpu_ACQUIRE_GL_OBJECTS     ;
		case CL_COMMAND_RELEASE_GL_OBJECTS               :return stat_gpu_RELEASE_GL_OBJECTS     ;
		case CL_COMMAND_READ_BUFFER_RECT                 :return stat_gpu_READ_BUFFER_RECT    ;
		case CL_COMMAND_WRITE_BUFFER_RECT                :return stat_gpu_WRITE_BUFFER_RECT   ;
		case CL_COMMAND_COPY_BUFFER_RECT                 :return stat_gpu_COPY_BUFFER_RECT    ;
		case CL_COMMAND_USER                             :return stat_gpu_USER         ;
		case CL_COMMAND_BARRIER                          :return stat_gpu_BARRIER      ;
		case CL_COMMAND_MIGRATE_MEM_OBJECTS              :return stat_gpu_MIGRATE_MEM_OBJECTS    ;
		case CL_COMMAND_FILL_BUFFER                      :return stat_gpu_FILL_BUFFER       ;
		case CL_COMMAND_FILL_IMAGE                       :return stat_gpu_FILL_IMAGE ;
		default:  return stat_gpu_other;
	}
}
...
static prl_mem prl_mem_create_empty(size_t size,const char *name,  prl_scop_instance scopinst) { assert(prl_initialized);
	assert(size>0);
...
static void prl_mem_alloc_host_only(prl_scop_instance scopinst, prl_mem mem) {
	...
	mem->host_mem = malloc_checked(scopinst, mem->size); mem->host_exposed=false;
...
static int get_int(const char *str) {	assert(str);
	char *end=NULL;
...
static void print_stat(double durations[static const restrict STAT_ENTRIES], int counts[const restrict STAT_ENTRIES], double relstddevs[const restrict STAT_ENTRIES], const char *prefix) {
	...
	if (global_state.config.cpu_profiling && global_state.config.gpu_profiling) puts("");
...
static void prl_release() {
    ...
    while (program) {
      ...
      if (program->program)  clReleaseProgram(program->program);
...
static void prl_init() {
    ...
    bool dumping = global_state.config.profiling_dump_on_release;
    if (dumping) {
	fputs("===============================================================================\n", stdout); puts("Initializing PRL...");
	fputs( "Profiling: ", stdout);
	if (global_state.config.cpu_profiling) fputs("CPU", stdout);
	if (global_state.config.cpu_profiling && global_state.config.gpu_profiling) fputs( ", ", stdout);
	if (global_state.config.gpu_profiling) fputs( "GPU", stdout);
	 puts("");
	}
...
prl_scop_instance prl_scop_enter(prl_scop *scopref) { assert(scopref);
    prl_init();
    ...
    struct prl_scop_inst_struct dummystat = {0};
      prl_scop_instance scopinst = malloc_checked(&dummystat, sizeof *scopinst); memset(scopinst,0,sizeof *scopinst);
...
prl_mem prl_scop_get_mem(prl_scop_instance scopinst, void *host_mem, size_t size, const char *name) {
	...
	lmem->host_writable = true; lmem->host_exposed = true;
	lmem->loc = loc_host;
...
static void ensure_dev_allocated(prl_scop_instance scopinst, prl_mem mem) {
	...
	mem->dev_writable = true; mem->dev_exposed = false;
...
void prl_scop_device_to_host(prl_scop_instance scopinst, prl_mem mem) {
    assert(scopinst);
    assert(mem);assert(is_valid_loc(mem));
    ...
    switch(mem->type) {
	case alloc_type_rwbuf:{cl_event event=NULL;
	...
	assert(mem->loc==loc_host|| mem->loc==loc_transferring_to_host);assert(is_valid_loc(mem));
}
